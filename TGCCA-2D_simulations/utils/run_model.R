...
run_svd <- function(dataset, raw_params = NULL, root_path = NULL, seed = 0) {
  a <- lapply(dataset$Xu, function(x) svd(x, nv = 1)$v)
  return(list(a = a))
}

run_twodcca <- function(dataset, raw_params = NULL, root_path = NULL, seed = 0) {
  require(reticulate)
  np <- import("numpy")

  params <- list(ranks = 1, tau = 0.001, scale_block = T, scale = F)
  mapping <- c("ranks", "tau", "scale_block", "scale")
  param_type <- c("num_vec", "num_vec", "bool", "bool")
  if (!is.null(raw_params)) {
    for (i in 1:length(raw_params)) {
      params[[mapping[i]]] <- parse_param(raw_params[i], param_type[i])
    }
  ...
  np$save(paste0(output_dir, "/X.npy"), r_to_py(blocks[[1]]))
  np$save(paste0(output_dir, "/Y.npy"), r_to_py(blocks[[2]]))

  script_path <- paste0(root_path, "/TCCA-youlinchen/run_twoDcca.py")
  system(paste0("python ", script_path, " -d ", output_dir, " -t ", params$tau), wait = TRUE)

  a <- list()
  fac1 <- np$load(paste0(output_dir, "/Lx.npy"))
  fac2 <- np$load(paste0(output_dir, "/Rx.npy"))
  a[[1]] <- fac2 %x% fac1
  fac1 <- np$load(paste0(output_dir, "/Ly.npy"))
  fac2 <- np$load(paste0(output_dir, "/Ry.npy"))
  a[[2]] <- fac2 %x% fac1
...
run_rtwodcca <- function(dataset, raw_params = NULL, root_path = NULL, seed = 0) {
  require(reticulate)
  require(multiway)
  np <- import("numpy")
  ...
  script_path <- paste0(root_path, "/TCCA-youlinchen/run_r_twoDcca.py")
  system(paste0("python ", script_path, " -d ", output_dir, " -t ", params$tau, " -r ", params$ranks), wait = TRUE)

  a <- list()
  fac1 <- np$load(paste0(output_dir, "/Lx.npy"))
  fac2 <- np$load(paste0(output_dir, "/Rx.npy"))
  a[[1]] <- apply(krprod(fac2, fac1), 1, sum)
  fac1 <- np$load(paste0(output_dir, "/Ly.npy"))
  fac2 <- np$load(paste0(output_dir, "/Ry.npy"))
  a[[2]] <- apply(krprod(fac2, fac1), 1, sum)
