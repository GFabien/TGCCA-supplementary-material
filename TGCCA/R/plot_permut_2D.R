...
plot_permut_2D <- function(perm,
  ...
  if (type == "zstat") {
    ...
    if (bars == "stderr") {
      tab <- aggregate(dft, by = list(dft[, 1]), function(x) {
    ...
    if (bars == "quantile") {
      tabq1 <- aggregate(dft, by = list(dft[, 1]), function(x) {
        return(quantile(x, 0.05))
      })
      tabq2 <- aggregate(dft, by = list(dft[, 1]), function(x) {
