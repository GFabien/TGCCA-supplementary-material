...
rgcca_predict <- function(rgcca_res,
  ...
  if (rgcca_res$call$method == "complete") {
    rgcca_res$blocks <- intersection_list(rgcca_res$blocks)
  }
  astar <- rgcca_res$astar

  # Dealing with vectors instead of matrices
  blocks_rgcca_res <- lapply(
    1:length(rgcca_res$blocks),
    function(i) {
      if (dim(rgcca_res$blocks[[i]])[2] == 0) {
        rgcca_res$blocks[[i]] <- matrix(rgcca_res$blocks[[i]], ncol = 1)
  ...
  if (!is.null(bloc_to_pred)) {
    if (mode(newA2[[bloc_to_pred]]) == "character") {
      if (length(unique(rgcca_res$call$raw[[bloc_to_pred]])) == 1) {
